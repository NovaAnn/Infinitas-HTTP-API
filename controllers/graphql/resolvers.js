...
module.exports = {
  ...
  getMyRequests: async function ({ workerId,status}, req) {
    ...
    if (!status){
       ...
       requests = await Request.find({workerId:workerId,status:status.toUpperCase()});
    }
  ...
  getMyWorklistRequests: async function ({ managerId,status}, req) {
    ...
    if (!status){
       ...
       worklistRequests = await Request.find({managerId:managerId,status:status.toUpperCase()}).populate("workerId");
    }
  ...
  seeMyEmployees: async function ({ managerId}, req) {
    ...
    const employees = await Worker.find({managerId:managerId}).populate("requests.requestId").populate("manager");
    console.log("2");
  ...
  getOverlappingRequests: async function ({ managerId}, req) {
    ...
    const requests = await Request.find({managerId:managerId,status:"PENDING"}).populate("workerId");

    if (!requests) {
     const error = new Error("No items on your worklist/You are not a manager");
     error.code = 201;
     throw error;
     }

    const datesArray = requests.map((req,index)=> { 
      const startEndDate = [(new Date(req.startDate.toISOString())).getTime(), (new Date(req.endDate.toISOString())).getTime(),index];
      return startEndDate;
  });
  ...
  processRequest: async function ({ managerId,requestId,status}, req) {
    ...
    const request = await Request.findOne({_id:requestId,managerId:managerId,status:"PENDING"});
